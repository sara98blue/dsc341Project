...
x0 <- kidneyMeans %>% dplyr::select(imputed_age, imputed_pe, class)
x0

x0$imputed_pe=factor(x0$imputed_pe, levels=c(1,2), labels=c("Yes","No"))

k0 <- ggplot(x0, aes(x = imputed_age, y = imputed_pe, color = class)) + geom_jitter()

k0 + labs(title = "Age and pe in patients with and with out chronic kidney disease", x ="patient age", y="pe")
...
x3 <- kidneyMeans %>% dplyr::select(imputed_al, imputed_su, class)
x3
...
k3 <- ggplot(x3, aes(x = imputed_al, y = imputed_su, color = class)) + geom_jitter()


k3 + labs(title = " chronic kidney disease", x ="Sugar", y = "Albunin")
...
x6 <- kidneyMeans %>% dplyr::select(imputed_rbcc,imputed_wbcc, class)
x6

k6 <- ggplot(x6, aes(x = imputed_rbcc, y = imputed_wbcc, color = class)) + geom_jitter(width = .1)


k6 + labs(title = "Pot and sodium with and without chronic kidney disease", x ="Red", y = "White") ##+xlim(100,160) + ylim(0,10)
...
table(kidneyMeans$class) # compute table of counts
t2=table(kidneyMeans$class)

barplot(t2, main="Chronic Kideny Disease Status")

ggplot(kidneyMeans, aes(x=imputed_sc)) + geom_histogram(binwidth=.5, colour="black", fill="white") + facet_grid(class ~ .) + labs(title = "Age with and without chronic kidney disease", x ="age")
##summary(kidneyMeans$class)
